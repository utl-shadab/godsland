...
const ItemHistory = ({ className }: ItemHistoryProps) => {
    ...
    return (
        <div className={`w-full ${className}`}>
            <h2 className="text-lg md:text-xl font-bold text-white mb-4 md:mb-6">History</h2>
            ...
            <div className="flex gap-4 md:gap-6 border-b border-white/10 mb-4 md:mb-6">
                {(['Sales', 'Offers', 'Rents'] as HistoryType[]).map((tab) => (
                    <button
                        ...
                        className={`pb-3 text-xs md:text-sm font-bold tracking-wide border-b-2 transition-colors duration-200 relative top-[1px] ${activeTab === tab
            ...
            <div className="bg-[#111] border border-white/5 rounded-xl overflow-hidden">
                ...
                <div className="overflow-x-auto">
                    <div className="min-w-[600px]">
                        {/* Table Header */}
                        <div className="grid grid-cols-5 gap-4 px-4 md:px-6 py-3 md:py-4 bg-white/5 border-b border-white/5 text-gray-400 text-[10px] md:text-xs font-bold uppercase tracking-wider">
                            ...
                            {currentData.length > 0 ? (
                                <div className="divide-y divide-white/5">
                                    {currentData.map((item, index) => (
                                        <div key={index} className="grid grid-cols-5 gap-4 px-4 md:px-6 py-3 md:py-4 items-center text-xs md:text-sm text-white hover:bg-white/[0.03] transition-colors duration-200">
                                ...
                                <div className="flex items-center justify-center h-[150px] text-gray-500 text-xs md:text-sm">
                                    Here is nothing
                                </div>
                            )}
                        </div>
